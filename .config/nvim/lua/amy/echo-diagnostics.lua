...
M.echo_line_diagnostic = function()
    if M.find_line_diagnostic(false) then
        return
    end
    vim.api.nvim_echo({ { M.find_line_diagnostic(false) } }, false, {})
    cmd([[autocmd CursorMoved * ++once echo " "]])
end

M.echo_entire_diagnostic = function()
    if M.find_line_diagnostic(true) then
        return
    end
    vim.api.nvim_echo({ { M.find_line_diagnostic(true) } }, false, {})
    cmd([[autocmd CursorMoved * ++once echo " "]])
end
...
vim.api.nvim_create_autocmd("CursorHold", { callback = function()  require('echo-diagnostics').echo_line_diagnostic() end, })
